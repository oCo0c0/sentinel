...
上述命令中我们指定几个 JVM 参数，其中 `-Dserver.port=8089` 是 Spring Boot 的参数，
用于指定 Spring Boot 服务端启动端口为 `8089`。其余几个是 Sentinel 客户端的参数。
...
| 参数                                               | 作用 |
|--------------------------------------------------|--------|
| `-Dcsp.sentinel.dashboard.server=localhost:8089` |向 Sentinel 接入端指定控制台的地址|
| `-Dproject.name=sentinel-dashboard`              |向 Sentinel 指定应用名称，比如上面对应的应用名称就为 `sentinel-dashboard`|

全部的配置项可以参考 [启动配置项文档](https://github.com/alibaba/Sentinel/wiki/%E5%90%AF%E5%8A%A8%E9%85%8D%E7%BD%AE%E9%A1%B9)。

经过上述配置，控制台启动后会自动向自己发送心跳。程序启动后浏览器访问 `localhost:8089` 即可访问 Sentinel 控制台。
